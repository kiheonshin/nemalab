...
function LabPreview({
  ...
  return <canvas ref={canvasRef} className={styles.previewCanvas} aria-label="Lab preview canvas" />;
}
...
export function ConnectomeView() {
  ...
  const liveFrameRef = useRef<ReturnType<typeof buildConnectomeFrame> | null>(null);
  const liveSnapshotRef = useRef<Snapshot | null>(null);
  const [selectedNeuron, setSelectedNeuron] = useState('AVBL');
  const [frame, setFrame] = useState<ReturnType<typeof buildConnectomeFrame> | null>(null);
  ...
  const previewStateLabel = snapshot
    ? snapshot.state === 'cruise'
      ? isKorean
        ? '전진'
        : 'Cruise'
      : snapshot.state === 'reverse'
        ? isKorean
          ? '후진'
          : 'Reverse'
        : isKorean
  ...
  const summaryNeurons: ConnectomeNeuronActivity[] = frame?.topNeurons.slice(0, 8) ?? [];

  return (
    <PageLayout title="" hideHeader contentClassName={styles.pageContent}>
      {!frame || !simInstance || !displaySnapshot || !snapshot ? (
        <div className={styles.emptyState}>
          <h2>{isKorean ? 'Connectome 준비 중' : 'Preparing Connectome'}</h2>
          <p>
            {isKorean
              ? '실험 회로 시뮬레이션을 초기화하고 있습니다.'
        ...
        <div className={styles.layout}>
          <aside className={styles.leftRail}>
            <section className={styles.infoSection}>
              <div className={styles.sectionHeader}>
                <span className={styles.sectionTitle}>{isKorean ? '실시간 입력' : 'Live Inputs'}</span>
                <span className={styles.sectionMeta}>{stateLabel}</span>
              </div>
            ...
            <section className={styles.infoSection}>
              <div className={styles.sectionHeader}>
                <span className={styles.sectionTitle}>
                  {isKorean ? '행동 스냅샷' : 'Behavior Snapshot'}
                </span>
              </div>
              <div className={styles.metricGrid}>
                <div className={styles.metricCard}>
                  <span className={styles.metricCardLabel}>{isKorean ? '먹이 근접도' : 'Food proximity'}</span>
                  <span className={styles.metricCardValue}>{Math.round(frame.foodProximity * 100)}%</span>
                </div>
                <div className={styles.metricCard}>
                  <span className={styles.metricCardLabel}>{isKorean ? '속도' : 'Speed'}</span>
                  <span className={styles.metricCardValue}>{displaySnapshot.speed.toFixed(1)} u/s</span>
                </div>
                <div className={styles.metricCard}>
                  <span className={styles.metricCardLabel}>{isKorean ? '실행 시간' : 'Elapsed'}</span>
                  <span className={styles.metricCardValue}>{displaySnapshot.metrics.elapsed.toFixed(1)} s</span>
                </div>
                <div className={styles.metricCard}>
                  <span className={styles.metricCardLabel}>{isKorean ? '센서 바이어스' : 'Sensor bias'}</span>
            ...
            <section className={styles.infoSection}>
              <div className={styles.sectionHeader}>
                <span className={styles.sectionTitle}>{isKorean ? '우세 회로' : 'Dominant Route'}</span>
              </div>
            ...
            <div className={styles.atlasStage}>
              <div className={styles.atlasCanvas}>
                <svg
                  ...
                  role="img"
                  aria-label="C. elegans connectome atlas"
                >
                  <rect
                    x="54"
                    y={REGION_Y}
                  ...
                  <g>
                    <text x="179" y={AXIS_LABEL_Y} textAnchor="middle" className={styles.svgAxisLabel}>
                      Head sensory ring
                    </text>
                    <text x="500" y={AXIS_LABEL_Y} textAnchor="middle" className={styles.svgAxisLabel}>
                      Integration + command
                    </text>
                    <text x="811" y={AXIS_LABEL_Y} textAnchor="middle" className={styles.svgAxisLabel}>
                      Motor output
                    </text>
                    <text x="80" y={FOOT_LABEL_Y} className={styles.svgFootLabel}>Head</text>
                    <text x="902" y={FOOT_LABEL_Y} textAnchor="end" className={styles.svgFootLabel}>
                      Tail
                    </text>
                  </g>
                </svg>
          ...
          <aside className={styles.rightRail}>
            <section className={styles.infoSection}>
              <div className={styles.sectionHeader}>
                <span className={styles.sectionTitle}>{isKorean ? '현재 행동 장면' : 'Current scene'}</span>
                <span className={styles.statePill}>{previewStateLabel}</span>
              </div>
              <div className={styles.previewFrame}>
                <LabPreview simInstance={simInstance} snapshot={snapshot} />
              </div>
            </section>

            <section className={styles.infoSection}>
              <div className={styles.sectionHeader}>
                <span className={styles.sectionTitle}>{isKorean ? '상위 활성 뉴런' : 'Top Active Neurons'}</span>
                <span className={styles.sectionMeta}>{isKorean ? '실시간 상위 8개' : 'Live top 8'}</span>
              </div>
            ...
            {selectedInfo ? (
              <section className={styles.infoSection}>
                <div className={styles.sectionHeader}>
                  <span className={styles.sectionTitle}>{isKorean ? '선택 뉴런' : 'Selected Neuron'}</span>
                </div>
                ...
                {selectedInfo.entry ? (
                  <div className={styles.selectedMetaGrid}>
                    <div className={styles.metaItem}>
                      <span className={styles.metaLabel}>Ganglion</span>
